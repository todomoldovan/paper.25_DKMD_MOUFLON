...
def plot_scalability_1x2(df_size, df_density, filename="figure2"):
	...
	# Show only actual sizes as ticks
	axL.set_xticks(sorted(df_size["nodes"].unique()))
	axL.margins(x=0.02)

	# --- Right: density (Edge probability)
	for strategy in strategies:
...
def plot_mouflon_alpha(net_node, net_full, filename="figure3"):
	...
	inset_ymin, inset_ymax = get_ncomms_limits([df1, df2], strategies=["base", "fexp", "step2", "hybrid"])

	metric_styles = {
		"modularity": STYLE["modularity"],
		"fair_exp": STYLE["prop_mouflon"],   # hybrid = MOUFLON (prop_balance)
		"fair_bal": STYLE["balance_mouflon"],# step2 = MOUFLON (balance)
	}
...
def plot_mouflon_psensitive(file_list1, file_list2, filename="figure4"):
	...
	metric_styles = {
		"modularity": STYLE["modularity"],
		"fair_exp": STYLE["prop_mouflon"],
		"fair_bal": STYLE["balance_mouflon"],
	}

	def plot_panel(ax, df):
		for metric, style in metric_styles.items():
			ax.plot(df["p_sensitive"], df[metric], **style)
...
##############################################
# Strategy configs (only MOUFLON variants now)
##############################################

strategy_config = {
	"step2": {
		"label": "MOUFLON (balance)",
		"fairness": "fair_bal",
		"style": STYLE["balance_mouflon"],
	},
	"hybrid": {
		"label": "MOUFLON (prop_balance)",
		"fairness": "fair_exp",
		"style": STYLE["prop_mouflon"],
	},
}
...
def main():
	# Scalability (Figure 2) → 1x2 (nodes vs density)
	...
	plot_scalability_1x2(main_df, main_df1, filename="figure2")

	# Files for p_sensitive experiments (used in Figures 4 & 6)
	node_files = [
		f"{log_path}/quality_new/nodex10/color-node_1000_r01_K2_c{p_sens}.csv"
		for p_sens in ["01", "02", "03", "04", "05"]
	]
	full_files = [
		f"{log_path}/quality_new/fullx10/color-full_1000_r01_K2_c{p_sens}.csv"
		for p_sens in ["01", "02", "03", "04", "05"]
	]
	...
	plot_multiple_alpha_real(
		["facebook_final", "deezer_final", "twitch_8graphs",
		 "pokec-a_8graphs_thresh_chng", "pokec-g"],
		draw_error=True,
		filename="figureB1"
	)
